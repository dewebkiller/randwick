...
class KlarnaCheckout extends Core {
	...
	function run() {
		...
		if ( empty($available_gateways[$this->klarna_id]) ) {
			return;
		}
		...
		if( $klarna_gateway ) {
			...
			add_action('cartimize_checkout_loaded_pre_head', array($this, 'klarna_pay_clicked'), 9);
		}

        add_filter('cartimize_load_checkout_template', array($this, 'detect_confirmation_page'), 10, 1);
        add_action('cartimize_checkout_loaded_pre_head', array($this, 'klarna_template_hooks'), 10);
	}

	function klarna_pay_clicked() {
		if(isset($_GET["payment_method"]) && $_GET["payment_method"] == "kco") {
	...
	function klarna_template_hooks() {
		if ( $this->is_klarna_payment_selected() ) {
			...
			add_filter( 'cartimize_replace_form', '__return_true' );
			add_action( 'cartimize_checkout_form', array( $this, 'klarna_checkout_form' ) );
		} else {
			// add_action('cartimize_checkout_add_separator', array( $this, 'add_separator' ), 11 );
			// add_action('cartimize_payment_request_buttons', array($this, 'add_klarna_pay_button'));
		}
	}
...
<path d="M79.7,0H57.4c0,18.3-8.4,35-23,46l-8.8,6.6l34.2,46.6h28.1L56.4,56.3C71.3,41.5,79.7,21.5,79.7,0z"/>
	...
	function detect_confirmation_page($load) {
		if (!empty($_GET['confirm']) && !empty($_GET['kco_wc_order_id'] ) || isset($_GET["payment_method"]) && $_GET["payment_method"] == "kco") {
			return false;
		}
